...
describe("Manual Rating Widget recording tests, old call ", () => {
    it("Checks if a rating object is send correctly", () => {
        hp.haltAndClearStorage(() => {
            initMain();
            Altruist.recordRatingWidgetWithID(ratingMaker("123", 1));
    ...
    it("Checks if rating recording without id would be stopped", () => {
        hp.haltAndClearStorage(() => {
            initMain();
            Altruist.recordRatingWidgetWithID(ratingMaker(undefined, 1));
            cy.fetch_local_event_queue().then((eq) => {
                cy.log(eq);
    ...
    it("Checks if rating recording without rating would be stopped", () => {
        hp.haltAndClearStorage(() => {
            initMain();
            Altruist.recordRatingWidgetWithID(ratingMaker("123", undefined));
            cy.fetch_local_event_queue().then((eq) => {
                cy.log(eq);
                expect(eq.length).to.equal(0);
            });
    ...
    it("Checks if id and rating is enough", () => {
        hp.haltAndClearStorage(() => {
            initMain();
            Altruist.recordRatingWidgetWithID({ widget_id: "123", rating: 1 });
            cy.fetch_local_event_queue().then((eq) => {
                cy.log(eq);
                expect(eq.length).to.equal(1);
                cy.check_commons(eq[0], 1);
    ...
    it("Check improper rating number in fixed", () => {
        hp.haltAndClearStorage(() => {
            initMain();
            Altruist.recordRatingWidgetWithID({ widget_id: "123", rating: 11 });
            cy.fetch_local_event_queue().then((eq) => {
                cy.log(eq);
                expect(eq.length).to.equal(1);
                cy.check_commons(eq[0], 1);
